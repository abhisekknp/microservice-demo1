...
* [Abhishek Omar](https://www.linkedin.com/in/abhisek-omar-6208168/)
...
* Run the individual images as below
    ...
    * Eureka Server
        * docker run -d --name registry-server -p 8761:8761 anilallewar/basic-webservice-registry:0.0.1
        * docker logs -f registry-server
    * OAuth Server
        * docker run -d --name auth-server -p 8899:8899 anilallewar/basic-auth-server:0.0.1
        * docker logs -f auth-server
    * Zuul API Gateway    
        * docker run -d --name api-gateway -p 8080:8080 anilallewar/basic-api-gateway:0.0.1
